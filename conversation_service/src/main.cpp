...
int main(int argc, char* argv[]) {
    ...
    nebula::ConversationServiceContext context;
    if (!context.init(config_path)) {
        LOG_ERROR("failed to init ConversationServiceContext");
        return 1;
    }
    nebula::ConversationServiceImpl service(context.conversationDao());
    auto credentials = nebula::GrpcTlsCredentials::serverCredentials(context.grpcTlsConfig());
    if (!credentials) {
        LOG_ERROR("failed to initialize ConversationService gRPC credentials");
        return 1;
    }
    ...
    if (!server) {
        LOG_ERROR("failed to start ConversationService on " + context.listenAddress());
        return 1;
    }
    ...
    std::cout << "nebula_conversation_service placeholder ready, config=" << config_path << std::endl;
    std::cout << "gRPC/storage disabled: install dependencies to enable ConversationService" << std::endl;
    return 0;
#endif
}
