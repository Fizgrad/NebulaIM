...
int main(int argc, char* argv[]) {
    ...
    nebula::GatewayServer tcp_server(&loop, listen_addr, options.gateway_id, context.connectionManager(), context.onlineManager(), context.router(), options.tcp_worker_threads, options.heartbeat_timeout_ms);
    tcp_server.start();
    ...
    LOG_INFO("Gateway TCP listening on " + options.tcp_host + ":" + std::to_string(options.tcp_port));
