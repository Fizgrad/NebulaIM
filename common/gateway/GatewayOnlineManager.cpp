...
bool GatewayOnlineManager::setOnline(uint64_t user_id, const std::string& connection_id) {
    if (redis_client_ == nullptr || user_id == 0 || connection_id.empty()) return false;
    return setOnline(user_id, connection_id, connection_id);
}
...
bool GatewayOnlineManager::setOnline(uint64_t user_id, const std::string& device_id, const std::string& connection_id) {
    ...
    bool legacy = redis_client_->setEx(onlineKey(user_id), online_ttl_seconds_, gateway_id_) &&
                  redis_client_->setEx(connKey(user_id), online_ttl_seconds_, connection_id);
    bool multi = redis_client_->setEx(onlineKey(user_id, device_id), online_ttl_seconds_, gateway_id_) &&
                 redis_client_->setEx(connKey(user_id, device_id), online_ttl_seconds_, connection_id);
    return legacy && multi;
}

bool GatewayOnlineManager::refreshOnline(uint64_t user_id, const std::string& connection_id) {
    ...
    return redis_client_->expire(onlineKey(user_id), online_ttl_seconds_) && redis_client_->expire(connKey(user_id), online_ttl_seconds_);
}
...
bool GatewayOnlineManager::setOffline(uint64_t user_id, const std::string& connection_id) {
    if (redis_client_ == nullptr || user_id == 0) return false;
    auto conn = redis_client_->get(connKey(user_id));
    if (conn.has_value() && conn.value() != connection_id) return true;
    redis_client_->del(onlineKey(user_id));
    redis_client_->del(connKey(user_id));
    return true;
}
...
bool GatewayOnlineManager::setOffline(uint64_t user_id, const std::string& device_id, const std::string& connection_id) {
    ...
    setOffline(user_id, connection_id);
    return true;
}

std::string GatewayOnlineManager::onlineKey(uint64_t user_id) const { return "nebula:user:online:" + std::to_string(user_id); }
std::string GatewayOnlineManager::connKey(uint64_t user_id) const { return "nebula:user:conn:" + std::to_string(user_id); }
