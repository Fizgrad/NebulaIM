...
start_one(){ name=$1; bin=$2; port=$3; setsid nohup "$ROOT/build/$bin" --config "$CONFIG" > "$ROOT/logs/$name.log" 2>&1 < /dev/null & pid=$!; echo "$pid" > "$ROOT/run/$name.pid"; sleep 0.2; if ! kill -0 "$pid" 2>/dev/null; then echo "$name failed to start; see $ROOT/logs/$name.log" >&2; exit 1; fi; echo "$name started pid=$pid port=$port config=$CONFIG"; }
